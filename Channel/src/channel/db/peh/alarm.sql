SELECT * FROM (SELECT * FROM MESSAGE WHERE MESSAGE_NUM > (SELECT MAX(MESSAGE_NUM) FROM MESSAGE WHERE MEMBER_NUM = #{member_num}) AND CHANNEL_NUM IN (SELECT MESSAGEROOM_NUM FROM MESSAGE WHERE MEMBER_NUM = #{member_num}) ORDER BY MESSAGE_NUM DESC);
...
SELECT * FROM (SELECT * FROM CHAT WHERE CHAT_NUM > (SELECT CHAT_NUM FROM CHAT WHERE MEMBER_NUM = 24 AND ROWNUM = 1) ORDER BY CHAT_NUM DESC);
